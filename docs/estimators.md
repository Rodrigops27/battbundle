...
This document is based on the code paths currently present in this repository. Every claim below is traceable to code, benchmark wiring, or existing repo docs. If a statement is not stated directly in code comments and is instead a deduction from code flow, it is marked `inferred`.

## Inventory

### Estimators found

- `ROM-EKF` (`estimators/initKF.m`, `estimators/iterEKF.m`)  
  Public benchmark estimator via `Evaluation/runBenchmark.m`.
- `ROM-SPKF` (`estimators/initKF.m`, `estimators/iterSPKF.m`)  
  Present in `estimators/`, but not exposed by `runBenchmark.m` or the active scenario scripts.
- `ESC-SPKF` (`estimators/initESCSPKF.m`, `estimators/iterESCSPKF.m`)
- `ESC-EKF` (`estimators/initESCSPKF.m`, `estimators/iterESCEKF.m`)
- `EaEKF` (`estimators/initEaEKF.m`, `estimators/iterEaEKF.m`)
- `EacrSPKF` (`estimators/initESCSPKF.m`, `estimators/iterEacrSPKF.m`)
- `EnacrSPKF` (`estimators/initESCSPKF.m`, `estimators/iterEnacrSPKF.m`)
- `EDUKF` (`estimators/initEDUKF.m`, `estimators/iterEDUKF.m`)
- `EsSPKF` (`estimators/initEDUKF.m`, `estimators/iterEsSPKF.m`)
- `EbSPKF` (`estimators/iterEbSPKF.m`)  
  Repo init helpers are local functions named `initEbSpkf` inside benchmark scripts such as `Evaluation/runBenchmark.m`.
- `EBiSPKF` (`estimators/iterEBiSPKF.m`)  
  Repo init helpers are local functions named `initEbiSpkf` inside benchmark scripts such as `Evaluation/runBenchmark.m`.
- `Em7SPKF` (`estimators/Em7init.m`, `estimators/Em7SPKF.m`)  
  Repo init helpers are local functions named `initEm7Spkf` inside benchmark scripts such as `Evaluation/runBenchmark.m`.

### Files inspected

- Core implementations: `estimators/initKF.m`, `estimators/iterEKF.m`, `estimators/iterSPKF.m`, `estimators/initESCSPKF.m`, `estimators/iterESCSPKF.m`, `estimators/iterESCEKF.m`, `estimators/initEaEKF.m`, `estimators/iterEaEKF.m`, `estimators/initEDUKF.m`, `estimators/iterEDUKF.m`, `estimators/iterEsSPKF.m`, `estimators/iterEbSPKF.m`, `estimators/iterEBiSPKF.m`, `estimators/iterEacrSPKF.m`, `estimators/iterEnacrSPKF.m`, `estimators/Em7init.m`, `estimators/Em7SPKF.m`.
- Benchmark and result wiring: `Evaluation/runBenchmark.m`, `Evaluation/xKFeval.m`, `Evaluation/mainEval.m`, `Evaluation/ABestComp.m`.
- Nearby studies and tests: `Evaluation/initSOCs/sweepInitSocStudy.m`, `Evaluation/tests/runInjTest.m`, `Evaluation/NoiseTuningSweep/sweepNoiseStudy.m`, `Evaluation/NoiseTuningSweep/oneEstSweeNoise.m`, `Evaluation/NoiseTuningSweep/plotEaEkfCovarianceSweeps.m`.
...
#### Short purpose
Two-stage ESC-SPKF with a separate external bias filter branch.
