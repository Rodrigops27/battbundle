...
% simulateROMProfile Master ROM simulator for simulated profile playback.
...
dataset.rom_file = toProjectRelativePath(rom_file, project_root);
dataset.esc_model_file = toProjectRelativePath(esc_model_file, project_root);
...
function path_out = toProjectRelativePath(path_in, project_root)
if isempty(path_in)
    path_out = '';
    return;
end
...
project_root = normalizeStoredPath(project_root);

if startsWith(lower(path_in), lower(project_root))
    path_out = path_in(numel(project_root)+1:end);
else
    path_out = path_in;
end
