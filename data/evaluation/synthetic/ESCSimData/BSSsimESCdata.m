...
repo_root = inferRepoRoot(script_dir);

addpath(repo_root);
addpath(genpath(fullfile(repo_root, 'utility')));
ensureDataRegistryLayout(repo_root, 'suite_versions', {'desktop_atl20_bss_v1'});

if nargin < 1 || isempty(savePath)
    savePath = fullfile(repo_root, 'data', 'evaluation', 'processed', ...
...
if ~isfield(cfg, 'profile_file') || isempty(cfg.profile_file)
    cfg.profile_file = fullfile(repo_root, 'data', 'evaluation', 'raw', ...
        'omtlife8ahc_hp', 'Bus_CoreBatteryData_Data.mat');
end
...
savePath = resolveEvaluationDatasetPath(savePath, repo_root, 'access', 'builder', 'must_exist', false);
...
path_out = relativizeRepoPath(path_in, inferRepoRoot(fileparts(mfilename('fullpath'))));
end
...
function repo_root = inferRepoRoot(script_dir)
repo_root = findRepoRoot(script_dir);
if isempty(repo_root)
    error('BSSsimESCdata:MissingRepoRoot', ...
        'Could not infer the repository root from %s.', script_dir);
end
end

function repo_root = findRepoRoot(start_dir)
repo_root = '';
...
local_path = resolveEvaluationDatasetPath(input_path, repo_root, 'access', 'builder', 'must_exist', true);
end
