function validation = BSSProfile_Validation(cfg)
% BSSProfile_Validation Validate a source profile after ROM dataset generation.
%
% Usage:
%   validation = BSSProfile_Validation()
%   validation = BSSProfile_Validation(cfg)
%
% Useful cfg fields:
%   profile_file       Source profile to map into the ROM simulation path.
%   dataset_file       Target MAT file storing the generated ROM dataset.
%   source_capacity_ah Source-cell nominal capacity.
%   tc                 Assumed temperature in degC for the ROM simulation.
%   rebuild_dataset    Rebuild the ROM dataset even if the MAT file exists.
%   show_plots         Show comparison plots. Default true.
%   validation_name    Label used in printed summaries and figure titles.
%
% Optional pass-through fields for createBusCoreBatterySyntheticDataset:
%   source_field_map, current_sign, original_capacity_ah,
%   original_1c_current_a, soc_init
...
if nargout == 0
    assignin('base', 'profileValidation', validation);
end
end

function cfg = normalizeConfig(cfg, romsim_root, repo_root)
...
cfg.source_capacity_ah = getCfg(cfg, 'source_capacity_ah', 8);
...
cfg.dataset_file = resolveOutputPath(cfg.dataset_file, romsim_root);
...
if ~need_build
    ...
    if ~isfield(loaded, 'dataset')
        error('BSSProfile_Validation:BadDatasetFile', ...
...
if need_build
    build_cfg = buildDatasetConfig(cfg);
    dataset = createBusCoreBatterySyntheticDataset(cfg.dataset_file, build_cfg);
end
end

function build_cfg = buildDatasetConfig(cfg)
build_cfg = struct();
build_cfg.profile_file = cfg.profile_file;
build_cfg.source_capacity_ah = cfg.source_capacity_ah;
build_cfg.tc = cfg.tc;
...
rom_voltage = getOptionalVector(dataset, 'voltage_v');
validation.has_voltage = ~isempty(source_voltage) && ~isempty(rom_voltage);
if validation.has_voltage
    ...
    [validation.voltage_rmse, validation.voltage_mean_error, validation.voltage_max_abs_error] = ...
        calcErrorStats(source_voltage(valid_voltage) - rom_voltage(valid_voltage));
    if nnz(valid_voltage) >= 2
        corr_matrix = corrcoef(source_voltage(valid_voltage), rom_voltage(valid_voltage));
        validation.voltage_corr = corr_matrix(1, 2);
        validation.voltage_fit = polyfit(source_voltage(valid_voltage), rom_voltage(valid_voltage), 1);
    else
...
if validation.has_voltage
    ...
    fprintf('  Voltage correlation: %.4f | Fit: Vrom = %.4f * Vsrc + %.4f\n', ...
...
figure('Name', sprintf('Profile Validation - Current - %s', cfg.validation_name), ...
    'NumberTitle', 'off');
...
hold on;
plot(t, target_current, 'r--', 'LineWidth', 1.2, 'DisplayName', 'ROM-applied current');
grid on;
xlabel('Time [s]');
ylabel('Current [A]');
...
hold on;
plot(t, validation.target_c_rate, 'r--', 'LineWidth', 1.2, 'DisplayName', 'ROM-applied C-rate');
grid on;
xlabel('Time [s]');
ylabel('C-rate [-]');
...
if validation.has_soc
    ...
    figure('Name', sprintf('Profile Validation - SOC - %s', cfg.validation_name), ...
        'NumberTitle', 'off');
    tiledlayout(2, 1, 'TileSpacing', 'compact', 'Padding', 'compact');
    ...
    hold on;
    plot(t, 100 * soc_cc, 'b--', 'LineWidth', 1.3, 'DisplayName', 'ROM CC SOC');
    grid on;
    xlabel('Time [s]');
    ylabel('SOC [%]');
    ...
    xlabel('Time [s]');
    ylabel('SOC Error [%]');
    title(sprintf('SOC error: source - ROM CC (ME %.3f%%)', 100 * validation.soc_mean_error));
end
...
if validation.has_voltage
    ...
    hold on;
    plot(t, rom_voltage, 'r--', 'LineWidth', 1.3, 'DisplayName', 'ROM simulated voltage');
    grid on;
    xlabel('Time [s]');
    ylabel('Voltage [V]');
    ...
    title(sprintf('Voltage error: source - ROM (ME %.2f mV)', 1000 * validation.voltage_mean_error));

    nexttile;
    if any(valid_voltage)
        scatter(source_voltage(valid_voltage), rom_voltage(valid_voltage), 10, t(valid_voltage), 'filled');
        hold on;
        v_min = min([source_voltage(valid_voltage); rom_voltage(valid_voltage)]);
        v_max = max([source_voltage(valid_voltage); rom_voltage(valid_voltage)]);
        ...
        text(0.1, 0.5, 'No finite source/ROM voltage pairs available', 'Units', 'normalized');
        axis off;
...
if ~isfield(s, field_name) || isempty(s.(field_name))
    error('BSSProfile_Validation:MissingField', 'Dataset.%s is required.', field_name);
end
value = s.(field_name);
end
...
function repo_root = inferRepoRoot(start_dir)
repo_root = '';
current_dir = start_dir;
while ~isempty(current_dir)
    if exist(fullfile(current_dir, 'utility'), 'dir') == 7 && ...
...
if isempty(repo_root)
    error('BSSProfile_Validation:MissingRepoRoot', ...
        'Could not infer the repository root from %s.', start_dir);
end
end
