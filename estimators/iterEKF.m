...
%
% Inputs:
%   vk      = The present voltage-sensor measurement
%   ik      = The present current-sensor measurement
%   Tk      = The present temperature-sensor measurement
%   ekfData = The EKF data structure to be updated
%
% Output:
%   zk      = The present estimate of variables
%   boundzk = The bounds on those estimation errors
%   ekfData = The EKF data structure, updated for this iteration
%   Xind    = The models that were blended together for this iteration
%
% This function performs one iteration of the EKF. The ekfData structure
% must first be initialized by "initKF.m".
%
% Reference implementation adapted from "iterEKF.m", developed by:
%
...
if isempty(ind)
    ...
    % ----------------------ResidueUpdate---------------------------
    % Reconstruction gain/bias for the integrator-residue contribution to thetass0.
    ekfData.gNegThetass = ones(numel(ekfData.ind.negThetass),1);
    ekfData.bNegThetass = zeros(numel(ekfData.ind.negThetass),1);
    ekfData.gNegThetass(1) = 1; % Required for full charge test >1C
end

if warnCount > 10
    fprintf('EKF is probably broken/lost\n');
    warnCount = 0;
...
% This function computes all cell nonlinear output variables
    function [Vcell,Z,Zsoc] = getVariables(ik,Xind,T)
        ...
        % ---------------------------------------------------------------------
        % Step 1: Extract the state estimates of 4 nearest-neighbor models
        %         (needed for OB only)
        % ---------------------------------------------------------------------
        switch ekfData.method
            case 'OB'
        ...
        if SOCnAvg < 0, shortWarn('SOCnAvg < 0'); SOCnAvg = 1e-6; end
        if SOCnAvg > 1, shortWarn('SOCnAvg > 1'); SOCnAvg = 1-1e-6; end
        if SOCpAvg < 0, shortWarn('SOCpAvg < 0'); SOCpAvg = 1e-6; end
        if SOCpAvg > 1, shortWarn('SOCpAvg > 1'); SOCpAvg = 1-1e-6; end
        ...
        % ----------------------ResidueUpdate---------------------------
        gN = ekfData.gNegThetass(:);
        bN = ekfData.bNegThetass(:);
        tmp = Z(ind.negThetass) + gN*SOCnAvg + bN;
        % tmp = Z(ind.negThetass) + SOCnAvg; % original
        if any(tmp > 1)
            violIdx = find(tmp > 1);
            for ii = 1:numel(violIdx) % (1) Res-Integ, (2) 2D SurfSolid
                shortWarn(sprintf('negThetass(%d) > 1 (value = %.4f)', ...
                    violIdx(ii), tmp(violIdx(ii))));
                % ekfData.bNegThetass(violIdx(ii)) = 1-tmp(violIdx(ii))+0.014; 
            end
        elseif any(tmp < 0)
            shortWarn('negThetass < 0');
        end
        Z(ind.negThetass) = min(max(tmp, 1e-6), 1-1e-6);

        tmp = Z(ind.posThetass) + SOCpAvg;
        if any(tmp > 1)
            shortWarn('posThetass > 1');
        elseif any(tmp < 0)
            shortWarn('posThetass < 0');
        end
        Z(ind.posThetass) = min(max(tmp, 1e-6), 1-1e-6);
