...
%   4. EDUKF (dual ESC-SPKF with online R0 estimation)
%   5. EsSPKF (ESC model with simplified online R0 estimation)
%   6. EBiSPKF (ESC-SPKF with external bias tracking)
%   7. Em7SPKF (ESC-SPKF with external bias tracking + simplified R0-SPKF branch)

clear; 
% clc; close all;
clear iterEKF iterESCSPKF iterEDUKF iterEsSPKF iterEBiSPKF Em7SPKF;

script_fullpath = mfilename('fullpath');
if isempty(script_fullpath)
    script_fullpath = which('SOCestimatorsEval');
end
if isempty(script_fullpath)
    script_dir = pwd;
else
...
rom_file = firstExistingFile({ ...
    ...
    'SOCestimatorsEval:MissingROMFile', ...
...
esc_model_file = firstExistingFile({ ...
    ...
    'SOCestimatorsEval:MissingFullESCModel', ...
...
% -------------------------------------------------------------------------
% ESC Methods:
sigma_w  = 1e-3;    % 1e2 process/current noise, big = "trust the sensor"
...
edukf_esc = initEDUKF(soc_init, R0init, tc, SigmaX0, sigma_v, sigma_w, SigmaR0, ...
    SigmaWR0, nmc30_esc);

soc_edukf = NaN(n_samples, 1);
soc_edukf(1) = soc_init / 100;
v_edukf = NaN(n_samples, 1);
v_edukf(1) = OCVfromSOCtemp(soc_edukf(1), tc, nmc30_esc);
soc_edukf_bnd = NaN(n_samples, 1);
v_edukf_bnd = NaN(n_samples, 1);
r0_edukf = NaN(n_samples, 1);
r0_edukf(1) = edukf_esc.R0hat;
r0_edukf_bnd = NaN(n_samples, 1);
innov_pre_edukf = NaN(n_samples, 1);
sk_edukf = NaN(n_samples, 1);

for k = 2:n_samples
    [soc_edukf(k), v_edukf(k), soc_edukf_bnd(k), edukf_esc, v_edukf_bnd(k), ...
        r0_edukf(k), r0_edukf_bnd(k)] = iterEDUKF(rom_voltage(k), i_profile(k), tc, ts, edukf_esc);
    if isfield(edukf_esc, 'lastInnovationPre')
        innov_pre_edukf(k) = edukf_esc.lastInnovationPre;
    end
    if isfield(edukf_esc, 'lastSk')
        sk_edukf(k) = edukf_esc.lastSk;
    end

    soc_edukf(k) = max(0, min(1, soc_edukf(k)));
end
...
sigma_v  = 1e2;    % 1e-3 voltage noise, big = "trust the model more than the sensor"
% sigma_w  = 1e2;    % 1e2 process/current noise, big = "trust the sensor"
% sigma_v  = 1e-3;    % 1e-3 voltage noise, big = "trust the model more than the sensor"
...
v_error_edukf = rom_voltage - v_edukf;
...
rmse_edukf = sqrt(mean(error_edukf(~isnan(error_edukf)).^2));
...
v_rmse_edukf = sqrt(mean(v_error_edukf(~isnan(v_error_edukf)).^2));
...
max_error_edukf = max(abs(error_edukf(~isnan(error_edukf))));
...
fprintf('  EDUKF:                 RMSE = %.4f, Max Error = %.4f\n', rmse_edukf, max_error_edukf);
...
diag_edukf = printEstimatorBiasMetrics('EDUKF', error_edukf, v_error_edukf, innov_pre_edukf, sk_edukf);
...
if InnovationACFPACFfigs
    plotInnovationAcfPacf( ...
        {innov_pre_ekf, innov_pre_spkf, innov_pre_edukf, innov_pre_esspkf, innov_pre_ebispkf, innov_pre_em7}, ...
        {'ROM-EKF', 'ESC-SPKF', 'EDUKF', 'EsSPKF', 'EBiSPKF', 'Em7SPKF'}, ...
        60, ...
        'Pre-fit Innovation ACF/PACF (SOCestimatorsEval)');
end
...
plot(t, v_edukf, 'm-.', 'LineWidth', 1.5, 'DisplayName', 'EDUKF');
...
plot(t, 100*soc_edukf, 'm-.', 'LineWidth', 1.5, 'DisplayName', ...
    sprintf('EDUKF (RMSE=%.3f%%)', 100*rmse_edukf));
...
plot(t, 100*error_spkf, 'g:', 'LineWidth', 1.5, 'DisplayName', ...
    sprintf('ESC-SPKF (RMSE=%.3f%%)', 100*rmse_spkf));
plot(t, 100*error_edukf, 'm-.', 'LineWidth', 1.5, 'DisplayName', ...
    sprintf('EDUKF (RMSE=%.3f%%)', 100*rmse_edukf));
...
plot(t, v_error_edukf, 'm-.', 'LineWidth', 1.5, 'DisplayName', ...
    sprintf('EDUKF (RMSE=%.2f mV)', 1000*v_rmse_edukf));
...
if SOCfigs
    ...
    figure('Name', 'SOC Error (EDUKF)', 'NumberTitle', 'off');
    plot(100 * (trueSOC - soc_edukf)); hold on; grid on;
    set(gca, 'colororderindex', 1); plot(100 * soc_edukf_bnd, ':');
    set(gca, 'colororderindex', 1); plot(-100 * soc_edukf_bnd, ':');
    title(sprintf('SOC estimation error (percent, %s)', 'EDUKF'));

    count_edukf = sum(abs(trueSOC - soc_edukf) > soc_edukf_bnd, 'omitnan');
    if any(isnan(soc_edukf))
        fprintf(' - EDUKF failed (NaN estimates)\n');
    else
        fprintf(' - SOC estimate outside of bounds %g %% of the time (%s)\n', ...
            count_edukf / length(trueSOC) * 100, 'EDUKF');
    end
...
if Vfigs
    ...
    figure('Name', 'Voltage Error (EDUKF)', 'NumberTitle', 'off');
    plot(t, v_error_edukf); hold on; grid on;
    set(gca, 'colororderindex', 1); plot(t, +v_edukf_bnd, ':');
    set(gca, 'colororderindex', 1); plot(t, -v_edukf_bnd, ':');
    legend('Error', 'Bounds');
    title(sprintf('Voltage estimation error (%s)', 'EDUKF'));
...
plot(t, r0_edukf, 'm-', 'LineWidth', 1.5, 'DisplayName', 'EDUKF R0');
hold on;
...
plot(t, r0_edukf + r0_edukf_bnd, 'm:', 'HandleVisibility', 'off');
plot(t, r0_edukf - r0_edukf_bnd, 'm:', 'HandleVisibility', 'off');
...
title('R0 Estimates with Bounds (EDUKF vs EsSPKF vs Em7SPKF)');
legend('Location', 'best');
...
% results.soc_edukf = soc_edukf;
% results.voltage_ekf = v_ekf;
% results.voltage_spkf = v_spkf;
% results.voltage_edukf = v_edukf;
% results.r0_edukf = r0_edukf;
% results.bounds = struct( ...
%     'soc_ekf', soc_ekf_bnd, ...
%     'voltage_ekf', v_ekf_bnd, ...
%     'soc_spkf', soc_spkf_bnd, ...
%     'voltage_spkf', v_spkf_bnd, ...
%     'soc_edukf', soc_edukf_bnd, ...
%     'voltage_edukf', v_edukf_bnd, ...
%     'r0_edukf', r0_edukf_bnd);
% results.rmse = struct('cc', rmse_cc, 'ekf', rmse_ekf, 'spkf', rmse_spkf, 'edukf', rmse_edukf);
% results.max_error = struct('cc', max_error_cc, 'ekf', max_error_ekf, 'spkf', max_error_spkf, 'edukf', max_error_edukf);
% 
% results_file = fullfile(script_dir, 'results_SOC_estimators_eval.mat');
